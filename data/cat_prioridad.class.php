...
class cPrioridad extends BD
{
    ...
    public function getAllReg() {
        ...
        try{
            ...
            $query = "  SELECT id_prioridad_caso,
                               descripcion,
                               activo
                          FROM cat_prioridad 
                         WHERE 1 = 1 $condition
                         ORDER BY id_prioridad_caso DESC" . $milimite;
                        //  die($query);
    ...
    public function insertReg( $data ){
        ...
        try {
            $query_insert = "INSERT INTO cat_tipo_caso(id_usuario_captura, 
                                                       fecha_captura,
                                                       observaciones)
                                               VALUES (?,
                                                       ?,
    ...
    public function updateReg( $data ){
        ...
        $exec       = $this->conn->conexion();
        try{
            $update = "UPDATE cat_prioridad
                          SET descripcion       = ?    
                        WHERE id_prioridad_caso = ? ";
    ...
    public function deleteReg($id){
        $correcto = 2;
        try{
            $delete = "DELETE FROM cat_prioridad
                             WHERE id_prioridad_caso = $id";
